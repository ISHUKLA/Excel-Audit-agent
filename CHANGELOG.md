...
- Added `ReconciliationLine.calculation_evidence_status` and `.stale_cell_refs`, populated from every formula cell reachable in an authoritative output's derivation chain — not only the output cell itself — and propagated from the internal (`excel_vs_python`) line to its corresponding external (`python_vs_accounts`) line.
- `core/verdict_logic.compute_verdict` now caps any line whose evidence is `stale` or `unknown` at `incomplete`, evaluated before the numerical threshold/ambiguity logic — a zero delta, a zero-width threshold, or a generous threshold can no longer produce a pass on stale or freshness-unknown evidence. This applies identically to Gate 3's live preview and its final recompute, and — because staleness caps the line, not just the aggregate — it cannot be defeated at the internal, external, or overall level.
- Reviewer acknowledgement of an incomplete result (`acknowledge_incomplete`) continues to permit report preparation and PDF generation; it does not and cannot alter `calculation_evidence_status` or upgrade the verdict.
- Cached value, Python reconstruction, delta, delta percentage, and reconstruction coverage remain visible on a stale/unknown line — freshness is reported as a distinct fact from formula-reconstruction coverage, never conflated with it.
- UI (Gate 3) and the PDF report now name the affected authoritative output, list the specific stale/unknown cell references in its derivation, and state plainly that numerical agreement is not evidence of a fresh Excel calculation.
- Terminology correction: user-facing wording no longer implies "not flagged stale" means "verified fresh" or "freshly recalculated" — it states only that no known staleness indicator was detected under this prototype's rules.
- One-time, build-time LibreOffice 26.2.5.2 recalculation of two test fixtures (`tests/fixtures/clean.xlsx`, `tests/fixtures/reserves.xlsx`) and the three competition demonstration workbooks (`demo/workbooks/case_1...xlsx`, `case_2...xlsx`, `case_3...xlsx`, mirrored byte-for-byte into `outputs/ai2_2026_demo_pack_20260824/`), so their calculation mode is genuinely `automatic` rather than `unknown`. Every cached numeric value and the set of formula-bearing cells are unchanged before and after; the only textual differences are LibreOffice's own writer normalizing formula syntax (unnecessary sheet-name quoting, a single-cell range collapsed to a bare reference, `FALSE()` for bare `FALSE`) — semantically identical in every case, evidenced by identical cached values. Full hashes and per-file notes recorded in `demo/recalculation_provenance.json`. **This is a fixture-generation step only** — no recalculation engine was added to the running application, and no automated test invokes LibreOffice.
- No change to debit/credit orientation, control-total validation, workbook-hash controls, AI-transparency controls, deterministic formula semantics, mapping-approval requirements, or materiality calculations beyond the freshness cap described above.
