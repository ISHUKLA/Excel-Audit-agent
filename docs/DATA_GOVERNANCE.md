...
**Verification after restore.** A restored database should always be verified before it is trusted. Each retained row's hash covers its protected fields and depends on the row before it, so protected-field changes and removal, insertion, or reordering within the retained sequence break verification. `verify_chain()` returns whether that retained chain is internally intact and, if not, the specific point of divergence. It cannot by itself prove that the latest rows or whole database were not truncated or replaced; that requires `verify_against_checkpoint()` and an independently retained checkpoint created earlier with `record_checkpoint()`.
...
- **Tamper detection.** The audit log is **tamper-evident, not tamper-proof.** Database triggers reject direct `UPDATE`/`DELETE` at the SQL level. Independently, `verify_chain()` detects protected-field changes and removal or reordering within the retained sequence, including after those triggers are disabled. Without an externally retained checkpoint, it cannot prove that the latest rows or the database as a whole were not truncated or replaced. When such a checkpoint was recorded and retained separately, `verify_against_checkpoint()` compares the current row count and terminal hash to it. None of these controls prevents someone with direct file access from editing the database.
