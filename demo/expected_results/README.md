...
- `case_5_expected.json` — Every supported formula: pass / pass, with explicit unsupported boundaries.
- `case_6_expected.json` — Large reserve stress and solvency impact: pass / pass after mapping approval.
...
required human actions; and material limitations. `tests/test_demo_cases.py` and
`tests/test_demo_cases_5_6.py` assert that the pipeline's actual, deterministic
output matches these files.
