...
| Case 5 and Case 6 user-facing validation pack (`scripts/build_demo_cases_5_6.mjs`, `scripts/generate_demo_cases_5_6.py`, `scripts/finalize_demo_cases_5_6.py`, `scripts/benchmark_demo_case_6.py`, `demo/`, `benchmark/`, `outputs/ai2_2026_demo_pack_20260824/`, `demo_cases.py`, README and focused tests) | AI-written | Artifact Tool authoring; LibreOffice recalculation evidence; all 20 supported functions with explicit unsupported boundaries; 8,453-formula synthetic reserve-stress impact case; four-gate/PDF acceptance and reproducible benchmark, 2026-09-15 |
| Competition use-case evidence pack (`demo/final cases/`, `scripts/build_competition_cases.mjs`, `scripts/generate_competition_cases.py`, `scripts/finalize_competition_cases.py`, independent-challenge seal/version tools, anomaly-detector adjacent-row extension, README and focused tests) | AI-written | Cases 7–10 are four reproducible synthetic cases with Artifact Tool authoring and LibreOffice caches; formula inventories and SHA-256 manifests; separate internal/external gate evidence; Case 11 is protocol only with no independently authored workbook, 2026-09-15; relocated and renumbered after the existing six demo cases at user request |
| Competition-case Streamlit registry (`demo_cases.py`, `app.py`, focused demo/UI tests, README) | AI-written | Adds Cases 7a–10 to the existing demo loader, preserves Case 10's distinct workbook/reference contexts, and exposes Case 11 as a non-loadable protocol-only dropdown option, 2026-09-15 |
| Formula evidence refresh (`scripts/`, qualification fixtures, Case 5, Case 9 and Case 11 evidence, focused tests and README) | AI-written | Reconciles the public evidence with the 26-function catalogue: real exact XLOOKUP qualification via LibreOffice, all 26 Case 5 outputs, Case 9 SUMPRODUCT reconstruction with IRR remaining incomplete, and refreshed Case 11 scope handout, 2026-09-16 |
| Python runtime startup guard (`app.py`, `tests/test_app.py`) | AI-written | Fails early with an actionable Streamlit message when launched below Python 3.11, before importing modules that use Python 3.10+ annotation syntax; focused boundary and import-order tests, 2026-09-17 |
| Gate 4 canonical registry identity (`core/gates.py`, `agents/orchestrator.py`, `core/models.py`, `app.py`, report/README, and focused/end-to-end tests) | AI-written | Removes free-text approval roles, resolves one canonical CRO name and role from the local registry, fails closed on malformed/ambiguous entries, and visibly distinguishes local identity confirmation from authentication, 2026-09-17 |
| Step 14 — negative reserve bounds heuristic (`agents/anomaly_detector.py`, `tests/test_negative_reserve_bounds.py`, `tests/test_competition_cases_7_8.py`, README) | AI-written | `_detect_negative_reserve_bounds`: flags a formula cell with a negative cached value in a reserve/provision/liability/technical/ultimate context (tab or covering named range, never bare `cell_ref`), suppressed when the formula carries a reinsurance-related term; severity `warning`; known false positive on the Case 7 IFRS17 fixture (`Reserve Summary!B7`) documented in its test rather than hidden, 2026-09-18 |
...
| AGENTS.md | AI-written | This file, 2026-08-10; Step 14 and Group J evidence provenance entries added, 2026-09-18 |
